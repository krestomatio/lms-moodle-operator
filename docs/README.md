...
```bash
# All operators
kubectl apply -k https://github.com/krestomatio/lms-moodle-operator/config/operators?ref=v0.4.0
# Only LMS Moodle Operator
# kubectl apply -k https://github.com/krestomatio/lms-moodle-operator/config/default?ref=v0.4.0
```

2. **Configure a LMSMoodleTemplate:**
- Download and modify [this `LMSMoodleTemplate` sample](https://raw.githubusercontent.com/krestomatio/lms-moodle-operator/v0.4.0/config/samples/lms_v1alpha1_moodletemplate.yaml) file to define a lms moodle template. A ``LMSMoodleTemplate` can be use by one or many `LMSMoodle` resources as spec template.
```bash
curl -sSL 'https://raw.githubusercontent.com/krestomatio/lms-moodle-operator/v0.4.0/config/samples/lms_v1alpha1_moodletemplate.yaml' -o lms_v1alpha1_moodletemplate.yaml
...
```bash
# All operators
kubectl delete -k https://github.com/krestomatio/lms-moodle-operator/config/operators?ref=v0.4.0
# Only LMS Moodle Operator
# kubectl delete -k https://github.com/krestomatio/lms-moodle-operator/config/default?ref=v0.4.0
```
